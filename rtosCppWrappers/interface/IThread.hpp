...
public:
  ...
  /**
   * @brief   stops the running thread. And, returns the status of the thread
   * stop.
   *
   *          Every derived class can have its own implementation of this
   * function. This is provided so as to have a customized thread control. The
   * thread owner himself decides if the thread had to be stopped upon a
   * function call by some module. By default the Thread class will be evaluated
   * at the least possible ownership. The difference between the thread_delete
   ...
   *
   *          Returns the thread priority of the object the reference or the
   * pointer of this interface class is pointing to.
   *
   ...
   *
   *          Returns the current status of the thread. No state change to the
   * thread.
   *
   ...
   *
   *          Deletes the underling task and then. As the FreeRTOS is
   * implemented The resources are not unallocated till the control passes to
   * the idle task. This call fails when the task is locked by the owner task.
   * The difference between the thread_suspend and thread_delete is that here
   * the underling task is deleted and the object is destroyed.
   *
   * @return  rtos_return_status_e returns one of @ref rtos_return_status_e.
   */
  ...
  /**
   * @brief   Check if the thread is created or had issues while creating the
   * thread.
   *
