...
public:
  ...
  /**
   * @brief   Struct used to pass the notification value that encapsulates both
   * value and time-out status.
   */
  struct notify_value {
  ...
  Thread(name_t thread_name, /**<Single constructor for the thread class. */
         priority_t thread_priority, stack_size_t thread_stack_size,
         id_t thread_id = 0);
   ...
   *
   * @param   entryClearMask      MASK to clear the bits of notification value
   * on entry.
   * @param   exitClearMask       MASK to cleat the bits of notification value
   * on exit.
   * @param   msDelay             Time-out for the wait.
   * @param   notificationValue   Pointer to the memory that notify uses to
   * place the notification value.
   * @return  RET_STA_E           True if the notification is received well
   * before the time-out else false.
   */
  static RET_STA_E
  wait_for_notification(uint32_t entryClearMask, uint32_t exitClearMask,
                        delay_t msDelay,
                        uint32_t *pNotificationValue = nullptr);
  /**
   * @brief   Call to this method blocks the thread for the given time or until
   * the signal on specified bit is received which ever is sooner.
   *
   *          If the wait-time times out. The function returns times-out if not
   * the signal received is expected or not.
   *
   * @param   signalMask  32 bit mask. Bits set in uint32_t value to receive the
   * signal over.
   * @param   blockTime   Time for which the thread will be placed on blocked
   * list.
  ...
  /**
   * @brief   Call to this method blocks the thread for the given time or until
   * the signal on specified bit is received i.e forever.
   *
   * @param   signalMask  32 bit mask. Bits set in uint32_t value to receive the
   * signal over.
   * @return SIG_RET_VAL  returns one of the possible enum values.
  ...
  /**
   * @brief   Call to this method blocks the thread for the given time or until
   * the value is received which ever is sooner.
   *
