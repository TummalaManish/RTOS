...
TEST(MemoryManagerTestPositive, AllocatingStack){
    ...
    ----------- ACTION: ----------
    RTOS::eMemoryResult res = MemoryManagerMock::get_Instance().get_stack(stack,
size);
...
TEST(MemoryManagerTestPositive, AllocatingStack) {
  ...
  RTOS::eMemoryResult res =
      MemoryManagerMock::get_Instance().get_stack(stack, size);
...
TEST(MemoryManagerTestPositive, AllocatingByte) {
  ...
  uint8_t *p_Block;
  size_t size = 24 * 1000;
  RTOS::eMemoryResult res = MemoryManagerMock::get_Instance().get_block(
      reinterpret_cast<void **>(&p_Block), size);
...
TEST(MemoryManagerTestNegative, UnableToAllocateStack) {
  ...
  RTOS::stack_t stack;
  RTOS::stack_size_t size =
      24 * 1024; // Trying to reserve memory more than available.
  RTOS::eMemoryResult res =
      MemoryManagerMock::get_Instance().get_stack(stack, size);
...
TEST(MemoryManagerTestNegative, UnableToAllocateTask) {
  ...
  EXPECT_EQ(MemoryManagerMock::get_Instance().get_block(
                reinterpret_cast<void **>(&p_Temp), configTOTAL_HEAP_SIZE - 32),
