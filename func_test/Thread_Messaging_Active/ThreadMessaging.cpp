...
class ThreadM : public RTOS::Thread{
    ...
    [[noreturn]] void run() override{
        ...
        for(;;){
            ...
            RTOS::RET_STA_E msg_stat = m_slaveThread.send_value_with_no_over_write(VALUE_FOR_MESSAGE);
            if (msg_stat == RTOS::RET_STA_E::eRTOSFailure)
                std::cout<<"Expected failure as there is a value waiting to be read"<<std::endl;
            else
            ...
            if (msg_stat == RTOS::RET_STA_E::eRTOSSuccess)
                std::cout<<"Expected success as there is no value waiting to be read"<<std::endl;
            else
        ...
        for(;;){
            ...
            if (signal_stat.timed_out)
                ...
                if (signal_stat.received_value == VALUE_FOR_MESSAGE) {
                    std::cout <<"Correct value is received in time: "
                              <<std::hex
                              <<static_cast<uint32_t>(signal_stat.received_value)
                              <<std::endl;
                }
                else{
                    std::cout <<"In-correct value is received in time: "
                              <<std::hex
                              <<static_cast<uint32_t>(signal_stat.received_value)
                              <<std::endl;
                }
