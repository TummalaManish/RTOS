...
namespace RTOS {
    ...
    Queue::Queue(const base_t queue_length, const base_t item_length){
        ...
        if (MemoryManager::get_Instance().get_CB(&m_pQueueCB) == eMemAllocationSuccess &&
            MemoryManager::get_Instance().get_block((void**)(&m_pBuffer), queue_length*item_length) == eMemAllocationSuccess)
            m_pHandle = xQueueCreateStatic(queue_length, item_length, m_pBuffer, m_pQueueCB);
        else
            debug_break;
    }
    ...
    RET_STA_E Queue::place_item_at_front(const void *const pv_item_to_queue, delay_t wait_time) {
    ...
    RET_STA_E Queue::place_item_at_back(const void *const pv_item_to_queue, delay_t wait_time) {
