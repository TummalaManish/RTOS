...
namespace RTOS{
    ...
    public:
        ...
        TQueue(){
            ...
            m_pQueueCB = nullptr;
            m_pBuffer = nullptr;
            if (MemoryManager::get_Instance().get_CB(&m_pQueueCB) == eMemAllocationSuccess &&
                MemoryManager::get_Instance().get_block((void**)(&m_pBuffer), N*sizeof(T)) == eMemAllocationSuccess)
                m_pHandle = xQueueCreateStatic(N, sizeof(T), reinterpret_cast<uint8_t*>(m_pBuffer), m_pQueueCB);
            else
                debug_break;
        }
        ...
        /*-------------------------- Inherited methods ---------------------------*/
        RET_STA_E place_item_at_front(const void *const pv_item_to_queue, delay_t wait_time) override {
            auto ret_val = xQueueSendToFront(m_pHandle, pv_item_to_queue, pdMS_TO_TICKS(wait_time));
            return ret_val == pdTRUE ? RET_STA_E::eRTOSSuccess : RET_STA_E::eRTOSFailure;
        }

        void place_item_at_front_blocked(const void *const pv_item_to_queue) override {
            (void)place_item_at_front(pv_item_to_queue, wait_forever);
        }

        RET_STA_E place_item_at_back(const void *const pv_item_to_queue, delay_t wait_time) override {
            auto ret_val = xQueueSendToBack(m_pHandle, pv_item_to_queue, pdMS_TO_TICKS(wait_time));
            return ret_val == pdTRUE ? RET_STA_E::eRTOSSuccess : RET_STA_E::eRTOSFailure;
        }
